...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    if (user.role !== UserRolesEnum.CUSTOMER && user.role !== UserRolesEnum.ADMIN) {
      toast.info("Only customers and admins can access the cart.");
      router.push("/");
      return;
    }
  ...
  const calculateAdvancePayment = () => {
    if (!cart) return 0;
    return (cart.discountedTotal - cart.securityDepositTotal) * 0.4; // 40% advance
  };
  ...
  const calculateItemBreakup = (item: CartItem) => {
    ...
    if (extraHours > 0) {
      if (totalHours <= 28) {
        // For 24-28 hours: charge extra hours at 10% per hour
      ...
      // For >28 hours: extra hours are charged as additional full days (already included in weekday/weekend counts)
    }
  ...
  if (!user || (user.role !== UserRolesEnum.CUSTOMER && user.role !== UserRolesEnum.ADMIN)) {
    return null;
  }
