...
const calculateTotalCost = () => {
  ...
  if (bookingPeriod.extraHours > 0) {
   ...
   const subtotal = calculatedRent + extraHoursCost;
   ...
   const breakdown = {
     ...
     calculation: `${bookingPeriod.weekdayCount} weekday(s) × ₹${motorcycle.pricePerDayMonThu} + ${bookingPeriod.weekendCount} weekend day(s) × ₹${motorcycle.pricePerDayFriSun}${bookingPeriod.extraHours > 0 ? (bookingPeriod.totalHours > 28 ? ` + ${Math.ceil(bookingPeriod.extraHours)} extra hour(s) (charged as 1 additional day)` : ` + ${Math.ceil(bookingPeriod.extraHours)} extra hour(s) × ₹${Math.round((bookingPeriod.lastDayTypeForExtraHours === "weekend" ? motorcycle.pricePerDayFriSun : motorcycle.pricePerDayMonThu) * 0.10)}`) : ''}`
   };

   return {
     totalCost: totalWithGST * watchedQuantity,
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                            ...
                            <div className="space-y-1 text-xs">
                              ...
                              <div className="flex justify-between font-semibold border-t pt-1">
                                ...
                                <span>₹{breakdown.totalWithGST.toLocaleString()}</span>
                              </div>
                              
                              {watchedQuantity > 1 && (
                                <div className="flex justify-between font-semibold border-t pt-1">
                                  <span>Total for {watchedQuantity} bikes:</span>
                                  <span>₹{(breakdown.totalWithGST * watchedQuantity).toLocaleString()}</span>
                                </div>
                              )}
                            </div>
                          </div>
