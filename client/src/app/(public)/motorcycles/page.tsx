...
export default function MotorcyclesPage() {
  ...
  const loadMoreMotorcycles = useCallback(async () => {
    ...
    try {
      ...
      if (selectedCategory !== "All Categories") filters.categories = selectedCategory;
      if (cities.length > 0) filters.cities = cities.join("$");
      if (debouncedMinPrice > 0) filters.minPrice = debouncedMinPrice;
      if (debouncedMaxPrice > 0) filters.maxPrice = debouncedMaxPrice;

      await fetchMoreMotorcycles(filters);
      
      // Check if we have more data
      const totalLoaded = motorcycles.length + itemsPerPage;
      ...
      console.error("Failed to load more motorcycles:", error);
    } finally {
  ...
  }, [loadMoreMotorcycles, hasMoreData, isLoadingMore]);
  ...
  useEffect(() => {
    ...
    const getMotorcycles = async () => {
      ...
      await getAllMotorcycles(filters);
      
      // Reset infinite scroll state
      setCurrentPage(1);
      setHasMoreData(metadata?.total > itemsPerPage);
    };
            ...
            <div ref={observerTarget} className="w-full h-10 flex items-center justify-center">
              {isLoadingMore && (
                <div className="flex items-center space-x-2 text-muted-foreground">
                  ...
                  <span>Loading more motorcycles...</span>
                </div>
              )}
              {!hasMoreData && motorcycles.length > 0 && (
                <div className="text-center text-muted-foreground py-8">
                  <p className="text-lg font-medium">You've seen all available motorcycles!</p>
